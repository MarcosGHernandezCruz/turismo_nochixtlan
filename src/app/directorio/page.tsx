      ...
      <div className="bg-primary text-white py-20 border-b-4 border-gold">
        <div className="container mx-auto px-4 text-center max-w-4xl">
          ...
          <p className="text-slate-200 font-serif text-lg leading-relaxed max-w-2xl mx-auto">
            Consulte la oferta de hospedaje y establecimientos gastronómicos tradicionales en Asunción Nochixtlán.
          </p>
        </div>
      </div>
                  ...
                  <div className="relative h-44 overflow-hidden bg-gradient-to-br from-primary via-[#5B1320] to-primary p-6 flex flex-col justify-between border-b-2 border-gold">
                    <div className="flex justify-between items-start">
                      <div className="w-12 h-12 rounded-none bg-primary/80 border border-gold flex items-center justify-center text-gold shadow-md">
                        {isHospedaje ? <Hotel size={24} /> : <Utensils size={24} />}
                      </div>
                      <span className="bg-white/90 text-primary px-3 py-1 text-[10px] font-bold uppercase tracking-widest border border-gold shadow-sm">
                        {negocio.categoria}
                      </span>
                    </div>

                    <div>
                      <h4 className="text-white font-serif font-bold text-lg tracking-wide line-clamp-1">
                        {negocio.nombre}
                      </h4>
                      <p className="text-gold/90 text-xs font-serif uppercase tracking-wider">
                        Asunción Nochixtlán
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                    ...
                    <Link href={`/lugar/${negocio.id}`} className="mt-auto">
                      <Button variant="outline" className="w-full border-2 border-primary text-primary hover:bg-primary hover:text-gold rounded-none font-serif uppercase tracking-widest text-xs font-bold py-5 transition-colors cursor-pointer">
                        Ver Ficha Institucional <ArrowRight size={14} className="ml-2" />
                      </Button>
                    </Link>
                  </div>
                </div>
